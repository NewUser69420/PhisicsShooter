...
public class Grenade_Bullet_Visual : NetworkBehaviour
{
    ...
    {
        if (this.IsSpawned && base.IsServerStarted) { StartCoroutine(Wait()); StartCoroutine(Wait2()); }
    ...
    {
        if (base.IsClientStarted) if (this.IsSpawned && !fuze) { Invoke(nameof(TurnOffBullet), 0.01f); }
        if (base.IsServerStarted) if (this.IsSpawned && !fuze) { Invoke(nameof(DespawnBullet), 0.01f); }
    }
